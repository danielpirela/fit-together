...
import { type DayOfWeek, getCellStatus, type SharedCompletion } from "@/core/entities";
...
export function HabitGrid({ completions, months = 12, cellSize = 32, className }: HabitGridProps) {
    ...
    <div className={cn("overflow-x-auto", className)}>
        ...
        {weeks.map((week, weekIndex) => (
          <div key={weekIndex} className="flex flex-col gap-[2px]">
            {DAYS_OF_WEEK.map((dayOfWeek) => {
              ...
              return <HabitGridCell key={dayOfWeek} status={status} size={cellSize} />;
            })}
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-1 mt-2">
        ...
        {MONTH_NAMES.map((month) => (
          <div key={month} className="text-xs text-gray-400" style={{ width: cellSize * 4 + 12 }}>
            {month}
          </div>
        ))}
      </div>
